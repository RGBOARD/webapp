    ...
    def dequeue(self):
        if self.email is not None:
            if self.user.is_admin():
                ...
                result = queue_item_dao.get_random_item()

                if result is not None:
                    pixel_data, duration = result
                    return jsonify({
                        "pixel_data": pixel_data,
                        "duration": duration
                    }), 200
                else:
                    return jsonify(error="No scheduled items found in the queue."), 404
            else:
                return jsonify(error="Only admins can dequeue."), 403
        return jsonify(error="User is not authenticathed."), 401
